-- Cargo Type filter becomes multi-select on the frontend -- change
-- p_cargo_type from a single text value to a text[] across all three RPCs
-- that use it, matching with `= any(p_cargo_type)` instead of `=`. A null
-- or empty array means "no filter" (same as null did before).

drop function if exists public.polygons_kpis(text, text, text);
drop function if exists public.polygons_terminals_geojson(text, text, text);
drop function if exists public.polygons_berths_geojson(text, text, text);
...
language sql stable as $$
  ...
  terminal_agg as (
    ...
    where port_code in (select port_id from scope_ports)
      and (p_cargo_type is null or array_length(p_cargo_type, 1) is null or terminal_type = any(p_cargo_type))
  ),
  berth_agg as (
    select
...
language sql stable as $$
  ...
  where t.port_code in (select port_id from public.polygons_scope_port_ids(p_scope, p_value))
    and (p_cargo_type is null or array_length(p_cargo_type, 1) is null or t.terminal_type = any(p_cargo_type));
$$;
